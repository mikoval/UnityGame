...
public class YBotSimpleControlScript : MonoBehaviour
{
	...
	private bool rightFootPlaying = false;
	private bool leftFootPlaying = false;
	private int FootstepLeftCounter = 0;
	private int FootstepRightCounter = 0;
	...
	public GameObject snowParticles = null;
    ...
    {	
		...
		prevPos2 = prevPos;
		prevPos = currentPos;
		currentPos = rbody.transform.position;
		FootstepRightCounter--;
		FootstepLeftCounter--;
		...
		if( Input.GetKeyUp("space")){
			if (IsGrounded && !anim.GetCurrentAnimatorStateInfo(0).IsName("Jumping")) {
				
				anim.SetBool("Jump", true);
				Vector3 impulse = currentPos - prevPos3;
		...
		if (Input.GetMouseButtonDown (0) && ! anim.GetCurrentAnimatorStateInfo(1).IsName("Throw") ) {
			if (ball.transform.parent == null) {
				ball = Instantiate (ballObj, (anim.GetBoneTransform (HumanBodyBones.LeftHand).position - new Vector3 (0.0f, 0.2f, 0.0f)), new Quaternion ());
				ball.transform.parent = anim.GetBoneTransform (HumanBodyBones.LeftHand).transform;
	...
	void throwEvent(){
		...
		ball.GetComponent<Rigidbody> ().velocity = forward * 10;
	}
    ...
    void OnCollisionEnter(Collision collision)
    {
		...
		if (collision.transform.gameObject.tag == "ground"|| collision.transform.gameObject.tag == "snow" )
        {
            ++groundContacts;
	...
	void FootstepHandler(){
		...
		Physics.Raycast(ray, out hit, totalRayLen);



		float rightFoot = anim.GetFloat ("RightFoot");
		float leftFoot = anim.GetFloat ("LeftFoot");
		if (rightFoot > 0.6 && ! rightFootPlaying) {
			rightFootPlaying = true;
			if (hit.collider.gameObject.CompareTag("ground")){
				EventManager.TriggerEvent<PlayerFootstepsGravelEvent, Vector3> (this.transform.position);
			}
			else if (hit.collider.gameObject.CompareTag("snow")){
				Instantiate(snowParticles,  anim.GetBoneTransform(HumanBodyBones.RightFoot).position, new Quaternion());
				EventManager.TriggerEvent<PlayerFootstepSnowEvent, Vector3> (this.transform.position);
			}

		} else if (rightFoot < 0.6) {
			rightFootPlaying = false;
		}
		if (leftFoot > 0.6 && ! leftFootPlaying) {
			leftFootPlaying = true;
			if (hit.collider.gameObject.CompareTag("ground")){
				EventManager.TriggerEvent<PlayerFootstepsGravelEvent, Vector3> (this.transform.position);
			}
			else if (hit.collider.gameObject.CompareTag("snow")){
				Instantiate(snowParticles,  anim.GetBoneTransform(HumanBodyBones.LeftFoot).position, new Quaternion());
				EventManager.TriggerEvent<PlayerFootstepSnowEvent, Vector3> (this.transform.position);
			}
		} else if (leftFoot < 0.6) {
			leftFootPlaying = false;
		}
	}
    //This is a physics callback
	...
	/*
	 * void LeftFoot(){
		if(FootstepLeftCounter <= 0){
			FootstepLeftCounter = 15;
			EventManager.TriggerEvent<PlayerFootstepsEvent, Vector3>(this.transform.position);
		}
	}
	void RightFoot(){
		if (FootstepRightCounter <= 0) {
			FootstepRightCounter = 15;
			EventManager.TriggerEvent<PlayerFootstepsEvent, Vector3> (this.transform.position);
		}
	}
	*/
	void fallingCheck(){
		...
		if (isFalling)
		{
			...
			{
				if (hit.collider.gameObject.CompareTag("ground") ||hit.collider.gameObject.CompareTag("snow") )
				{       
